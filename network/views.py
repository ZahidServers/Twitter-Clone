...
class Edit(forms.Form):
    textarea = forms.CharField(widget=forms.Textarea(attrs={'rows': 1, 'cols': 152, 'rows':10}), label='')
def index(request):
...
def search(request):
    if request.method=='POST':
        ...
        searchresults=User.objects.filter(username__icontains=searchkey)
        return render(request, 'network/search.html', {'searchresults':searchresults, 'searchkey':searchkey})
